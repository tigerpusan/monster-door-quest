...
class DoorScene extends PositionComponent with HasGameReference<MonsterDoorGame> {
    ...
    if (result == ChoiceResult.clear) {
      ...
      unawaited(
        game.audioManager.playClear(
          milestoneStage: realmMilestones().contains(session.stage),
        ),
      );
    }
  }
  ...
  void render(Canvas canvas) {
    ...
    canvas.drawRect(
      Rect.fromLTWH(0, 0, size.x, size.y * .335),
      Paint()..color = const Color(0xF4FFF8E8),
    );
    ...
    _center(
      canvas,
      stageRealmLabel(session.stage),
      size.y * .158,
      13.8,
      const Color(0xFF52779A),
      FontWeight.w800,
    );
    _center(
      canvas,
      'STAGE ${stageInRealm(session.stage)}   '
      '${min(session.step + 1, session.route.length)} / ${session.route.length}',
      size.y * .190,
      14.5,
    ...
    final remain =
        (GameRules.playSeconds - elapsed).clamp(0.0, GameRules.playSeconds);
    _center(
      canvas,
    ...
    if (_feedbackTimer > 0) {
      final rr = RRect.fromRectAndRadius(
        Rect.fromLTWH(size.x * .43, size.y * .355, size.x * .14, 42),
    ...
    if (feedback.flashKind != FlashKind.none) {
      final alpha =
          (42 * (1 - feedback.elapsed / .25).clamp(0, 1)).round();
      canvas.drawRect(
        size.toRect(),
        Paint()
